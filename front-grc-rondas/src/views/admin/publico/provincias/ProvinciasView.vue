  ...
  <div>
    ...
    <el-table border fit :data="tableData" class="py-4" v-loading="listLoading">
      ...
      <el-table-column align="right" width="120">
        <template #header>
          <el-button type="primary" class="ache-background-color-template" :icon="Plus" @click="openFormCreate" v-if="!isActionDisabled('pub.provincias.crear')">
            Agregar
          </el-button>
        </template>
        <template #default="scope">
          <el-tooltip
              ...
              placement="top-start"
          >
            <el-icon size="20" style="margin-right: 10px; cursor: pointer;" @click="openFormEditar(scope.row.id)" v-if="!isActionDisabled('pub.provincias.actualizar')" color="#E3CB2DFF"><Edit /></el-icon>
          </el-tooltip>
          <slot v-if="!isActionDisabled('pub.provincias.eliminar')">
            <el-tooltip
                class="box-item"
                effect="dark"
                content="Desactivar"
