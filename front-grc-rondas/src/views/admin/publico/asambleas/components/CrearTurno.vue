...
<!--          <el-button type="primary" @click="showSelectRonderosModal" :icon="Plus">Agregar Ronderos</el-button>-->
          <el-button type="primary" @click="getRonderosTable" :icon="Plus">Agregar Ronderos</el-button>
        </div>
        <el-table
...
const form_turno = reactive({
  ...
  fecha: '',
  responsable_turno: '',
  tipo_reunion: 'Asamblea', // Asamblea o Vigilancia
})
...
const resetForm = () => {
  ...
  form_turno.tipo_reunion = 'Asamblea'
...
const getRonderosTable = () => {

  loadingTable.value = true;
  keywordByBase.base_id = form_turno.base_id
  baseResource.getRonderosByBase(keywordByBase)
      .then(response => {
        ...
        data.forEach(item => {
          ...
          detalleProgramacionronderos.value.push(rowRondero);
        });
        loadingTable.value = false;
      })
      .catch(error => {
