      ...
      <div>
        <el-form ref="formCreateEditSector" v-loading="loadingSaveDialogForm" :model="modelSector"
          ...
          <el-form-item label="Distrito" prop="distrito_id">
            <el-select ref="distrito_idField" v-model="modelSector.distrito_id" placeholder="Distrito" style="width: 100%" filterable>
              <el-option
...
export default {
  ...
  setup() {
    ...
    const openFormCreate = () => {
      ...
      nextTick(() => {
        fetchDistritos(); //cargar distritos
    ...
    const openFormEditar = async (id) => {
      await fetchDistritos(); //cargar distritos
    ...
    const fetchDistritos = async () => {
      // Crear un query para obtener todos los distritos activos
      const queryDistritos = {
        keyword: '',
        limit: 1000,
        page: 1
      };
      
      await distritoResource.list(queryDistritos)
        .then(response => {
          const { data } = response
